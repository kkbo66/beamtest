...
Parameter::Parameter()
{
  //string ParaPath = getenv("ECALBEAMTESTROOT");
  char cwd[1024];
  string ParaPath;
  if(getcwd(cwd,sizeof(cwd))!=NULL) ParaPath=string(cwd);
...
double Parameter::HitPosX(int n) const
{
  ...
  if(it!=m_HitPos.end()) return it->second.x();
  else return 99;
}
...
{
  auto it=m_HitPos.find(n);
  if(it!=m_HitPos.end()) return it->second.y();
  else return 99;
}
...
{
  auto it=m_HitPos.find(n);
  if(it!=m_HitPos.end()) return it->second.z();
  else return 99;
}
