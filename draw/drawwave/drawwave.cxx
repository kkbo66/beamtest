...
void drawwave(int index1, int index2, int num){
  ...
  TFile *fout = new TFile("/home/kkbo/beamtest/draw/figureroot/wave.root", "UPDATE");
    for(int i=0; i<n; i++){
...
int main(int argv,char* argc[])
{
   ...
   TFile *fout = new TFile("/home/kkbo/beamtest/draw/figureroot/wave.root", "RECREATE");
   delete fout;
   if(argv==2){
     for(int i=0; i<5; i++){
       for(int j=0; j<5; j++){
         drawwave(i+1, j+1, atoi(argc[1]));
       }
     }
     return 0;
   }
   else if(argv==4){
     drawwave(atoi(argc[1]), atoi(argc[2]), atoi(argc[3]));
     return 0;
   }
   else if(argv==3){
     drawwave(atoi(argc[1]), atoi(argc[2]), 1000000);
     return 0;
   }
   else if(argv==1){
     for(int i=0; i<5; i++){
       for(int j=0; j<5; j++){
         drawwave(i+1, j+1, 1000000);
       }
     }
     return 0;
   }
