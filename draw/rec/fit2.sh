...
para_num(){
  if test $# -lt 6; then
    echo "Please Input Energy & Mode & Hit cut & Hit num & Reconstruct num & Seed cut"
    exit 1
  fi
}
...
if [ ${mode} -eq 1 ]; then
  ...
  root -l -b -q "drawfitshower.cxx(\"${TXTPATH}${energy}MeVep.txt\",${energy},${hitcut},${hitnum},\"${reconnum}\",${seedcut})" > ${energy}MeVep_fit.log 
  ...
  root -l -b -q "drawfitshower.cxx(\"${TXTPATH}${energy}MeVepmode2.txt\",${energy},${hitcut},${hitnum},\"${reconnum}\",${seedcut})" > ${energy}MeVep_fit.log
  num=$(grep "Resolution" ${energy}MeVep_fit.log | sed -n 's/.*Resolution: *\([0-9.]\+\) %.*/\1/p')
  entries=$(grep "Event entries" ${energy}MeVep_fit.log | sed -n 's/.*Event entries: *\([0-9]\+\).*/\1/p')
  if grep -q "^${energy}\b " EnergyResolution.txt; then
