...
int main(int argc, char const *argv[]){
  ...
  for(int k=0;k<nEntries;k++){
    ...
    for(int i=0;i<25;i++){
      ...
      if(IsFitAmp) rechit.setEnergy(AmpHG_Fit->at(i),AmpLG_Fit->at(i),Para.Ratio(i),Para.HGSatuPoint(i),Para.HGNoise(i),Para.LGPedestal(i),Para.LGNoise(i),Para.LGMipPeak(i));
      else rechit.setEnergy(AmpHG_Peak->at(i),AmpLG_Peak->at(i),Para.Ratio(i),Para.HGSatuPoint(i),Para.HGPedestal(i),Para.HGNoise(i),Para.LGPedestal(i),Para.LGNoise(i),Para.LGMipPeak(i),r1);
    ...
    for(Citer=ClusterMap.begin();Citer!=ClusterMap.end();Citer++){
      ...
      if(etot>0) possum=possum*(1/etot);
      if(std::abs(possum.x()-(std::floor(possum.x() * 100000) / 100000))<epsilon) Citer->second.setPosition(TVector3(std::floor(possum.x() * 100000) / 100000,std::floor(possum.y() * 100000) / 100000,std::floor(possum.z() * 100000) / 100000));
      else Citer->second.setPosition(possum);

      // Calculate cluster second moment
      double sum=0;
      for (auto it = Citer->second.Begin(); it != Citer->second.End(); ++it) {
        TVector3 pos(it->second.FrontCenter());
        if(std::abs((pos - possum).Mag2())<epsilon) sum+=it->second.Energy()*0;

        else sum+=it->second.Energy()*((pos - possum).Mag2());
      }
      if(etot>0) sum/=etot;
